...
def m022_history_inventory():
    """Establish the longest strict common history ending before M021 data."""

    feature_sha = _require_m022_branch()
    output_dir = _ensure_baseline_path(M022_INVENTORY_DIR)
    if output_dir.exists():
        return {
            "ok": False,
            "reason": (
                "M022 inventory directory already exists; run "
                "m022_history_inventory_cleanup explicitly before re-export"
            ),
            "feature_branch": "strategy-parameter-research",
            "feature_sha": feature_sha,
            "path": str(output_dir.relative_to(REPO)),
...
symbols = ["EURUSD", "EURJPY", "GBPUSD", "GBPJPY", "USDJPY"]
probe_floor = datetime.fromisoformat("2010-01-01T00:00:00+00:00")
cutoff = datetime.fromisoformat("2026-09-25T00:00:00+00:00")

if not mt5.initialize(**_mt5_initialize_kwargs(mt5_config)):
    raise RuntimeError("MT5 initialize failed for read-only M022 history probe")
...
try:
    ...
    for symbol in symbols:
        ...
        if first_tick is None:
            raise RuntimeError(
                f"no positive Ask tick found for {symbol} from probe floor"
            )

        output[symbol] = {
            "first_positive_ask_tick_utc": (
                first_tick.isoformat().replace("+00:00", "Z")
            ),
            "initial_probe_valid_tick_count": valid_tick_count,
        }

    terminal_version = list(mt5.version()) if hasattr(mt5, "version") else None
    print(json.dumps({
...
'''
    ...
    if probe["exit_code"] != 0:
        return {
            "ok": False,
            "reason": "read-only M022 earliest-Ask probe failed",
            "feature_branch": "strategy-parameter-research",
            "feature_sha": feature_sha,
            "probe": probe,
            "wine_python": wine_python,
    ...
    try:
        ...
        raise RuntimeError("unable to parse M022 history probe") from exc

    first_ticks = []
    for symbol in M022_SYMBOLS:
        value = (
            probe_payload.get("symbols", {})
            .get(symbol, {})
            .get("first_positive_ask_tick_utc")
        )
        if not value:
            return {
                "ok": False,
                "reason": f"M022 probe missing earliest Ask for {symbol}",
                "probe": probe_payload,
            }
        first_ticks.append(value)
    ...
    raw_start = max(_parse_utc(value) for value in first_ticks)
    candidate_start = raw_start.replace(second=0, microsecond=0)
    if raw_start > candidate_start:
        candidate_start += timedelta(minutes=1)
    candidate_start_utc = (
        candidate_start.isoformat().replace("+00:00", "Z")
    )
    ...
    export = _run(
        [
            ...
            *M022_SYMBOLS,
            ...
            M022_CUTOFF_UTC,
            "--output-dir",
            str(M022_INVENTORY_DIR.relative_to(REPO)),
            "--include-tick-ask",
            "--tick-chunk-minutes",
            "1440",
            "--rate-chunk-days",
    ...
    if export["exit_code"] != 0:
        return {
            "ok": False,
            "reason": "read-only M022 historical inventory export failed",
            "feature_branch": "strategy-parameter-research",
            "feature_sha": feature_sha,
            "candidate_start_utc": candidate_start_utc,
            "probe": probe_payload,
    ...
    if not M022_INVENTORY_MANIFEST.is_file():
        return {
            "ok": False,
            "reason": "M022 inventory export completed without manifest.json",
            "feature_branch": "strategy-parameter-research",
            "feature_sha": feature_sha,
            "candidate_start_utc": candidate_start_utc,
            "probe": probe_payload,
...
manifest_path = Path("backtest_data/m022-history-inventory-raw/manifest.json")
...
def gap_summary(frame, expected_minutes):
    diffs = frame.index.to_series().diff().dropna()
...
for symbol in symbols:
    ...
    missing_ask = m1.index.difference(ask.index)
    if len(missing_ask):
        last_missing = missing_ask[-1]
        later = m1.index[m1.index > last_missing]
        suffix_start = later[0] if len(later) else None
    else:
        suffix_start = m1.index[0]

    native = dataset.native_timeframe_bars[symbol]
    starts = [m1.index[0]]
    if suffix_start is not None:
        starts.append(suffix_start)
    for tf in ("M5", "M15"):
        starts.append(native[tf].index[0])
    strict_start = max(starts) if suffix_start is not None else None
    if strict_start is not None:
        strict_starts.append(strict_start)

    files = manifest["symbols"][symbol]["files"]
    ask_meta = manifest["symbols"][symbol]["ask_m1"]
    per_symbol[symbol] = {
        "m1": {
            "rows": int(len(m1)),
            "first_bar_open_utc": iso(m1.index[0]),
        ...
        "ask_m1": {
            ...
            "sha256": ask_meta["sha256"],
            "missing_vs_bid_m1_rows": int(len(missing_ask)),
            "first_missing_vs_bid_m1_utc": (
                iso(missing_ask[0]) if len(missing_ask) else None
            ),
            "last_missing_vs_bid_m1_utc": (
                iso(missing_ask[-1]) if len(missing_ask) else None
            ),
            "complete_suffix_start_utc": (
                iso(suffix_start) if suffix_start is not None else None
            ),
            "valid_ticks": ask_meta.get("valid_ticks"),
            "spread_points_min": ask_meta.get("spread_points_min"),
            "spread_points_max": ask_meta.get("spread_points_max"),
        ...
        "strict_symbol_start_utc": (
            iso(strict_start) if strict_start is not None else None
        ),
    }

common_start = max(strict_starts) if len(strict_starts) == len(symbols) else None
coverage_after_common = {}
if common_start is not None:
    for symbol in symbols:
        m1 = dataset.m1_bars[symbol]
        ask = dataset.ask_m1_bars[symbol]
        scoped_m1 = m1.loc[m1.index >= common_start]
        scoped_ask = ask.loc[ask.index >= common_start]
        coverage_after_common[symbol] = {
            "m1_rows": int(len(scoped_m1)),
            "ask_m1_rows": int(len(scoped_ask)),
            "missing_ask_rows": int(
                len(scoped_m1.index.difference(scoped_ask.index))
            ),
            "m5_rows": int(
                len(
                    dataset.native_timeframe_bars[symbol]["M5"].loc[
                        dataset.native_timeframe_bars[symbol]["M5"].index
                        >= common_start
                    ]
                )
            ),
            "m15_rows": int(
                len(
                    dataset.native_timeframe_bars[symbol]["M15"].loc[
                        dataset.native_timeframe_bars[symbol]["M15"].index
                        >= common_start
                    ]
                )
            ),
...
print(json.dumps({
    ...
    "account_currency": manifest.get("account_currency"),
    "terminal_version": manifest.get("terminal_version"),
    "requested_range": manifest.get("requested_range"),
    "per_symbol": per_symbol,
    "strict_common_start_utc": (
        iso(common_start) if common_start is not None else None
    ),
    "strict_common_end_exclusive_utc": "2026-09-25T00:00:00Z",
    "coverage_after_common_start": coverage_after_common,
}, sort_keys=True))
'''
    inspect = _run(
        _native_command("-c", inspect_code),
    ...
    if inspect["exit_code"] != 0:
        return {
            "ok": False,
            "reason": "M022 exported inventory failed integrity inspection",
            "feature_branch": "strategy-parameter-research",
            "feature_sha": feature_sha,
            "candidate_start_utc": candidate_start_utc,
            "probe": probe_payload,
    ...
    try:
        ...
        raise RuntimeError("unable to parse M022 inventory inspection") from exc

    common_start = summary.get("strict_common_start_utc")
    coverage = summary.get("coverage_after_common_start", {})
    coverage_ok = bool(common_start) and all(
        coverage.get(symbol, {}).get("m1_rows", 0) > 0
        and coverage.get(symbol, {}).get("m5_rows", 0) > 0
        and coverage.get(symbol, {}).get("m15_rows", 0) > 0
        and coverage.get(symbol, {}).get("missing_ask_rows") == 0
        for symbol in M022_SYMBOLS
    )
    ...
    return {
        ...
        "feature_branch": "strategy-parameter-research",
        "feature_sha": feature_sha,
        "probe_floor_utc": M022_PROBE_FLOOR_UTC,
        "cutoff_utc": M022_CUTOFF_UTC,
        "candidate_start_utc": candidate_start_utc,
        "probe": probe_payload,
        "inventory": summary,
